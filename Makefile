...
# The build scripts clone quickestphp/app, and this var selects the
...
help:
	@echo "QuickestPHP Makefile"
	@echo "================"
	@echo ""
	@echo "release VERSION=x.y.z"
	@echo "  Create a new release of QuickestPHP. Requires the VERSION and GITHUB_USER, or GITHUB_TOKEN parameter."
	...
	@echo ""
	@echo "publish"
	@echo "  Publish the dist/quickestphp-VERSION.zip to github."
	@echo ""
	@echo "components"
	...
	@echo ""
	@echo "test"
	@echo "  Run the tests for QuickestPHP."
	@echo ""
...
# Version bumping & tagging for QuickestPHP itself
...
tag-release: guard-VERSION bump-version
	...
	git tag -s $(VERSION) -m "QuickestPHP $(VERSION)"
	git push $(REMOTE)
...
build/quickestphp: build
	git checkout $(VERSION)
	git checkout-index -a -f --prefix=build/quickestphp/
	git checkout -

dist/quickestphp-$(DASH_VERSION).zip: build/app build/quickestphp composer.phar
	mkdir -p dist
	@echo "Installing app dependencies with composer"
	# Install deps with composer
	cd build/app && php ../../composer.phar install
	# Copy the current quickestphp libs up so we don't have to wait
	# for packagist to refresh.
	rm -rf build/app/vendor/quickestphp/quickestphp
	cp -r build/quickestphp build/app/vendor/quickestphp/quickestphp
	...
	cd build/app && find . -not -path '*.git*' | zip ../../dist/quickestphp-$(DASH_VERSION).zip -@

# Easier to type alias for zip balls
package: clean dist/quickestphp-$(DASH_VERSION).zip
...
publish: guard-VERSION guard-GITHUB_USER dist/quickestphp-$(DASH_VERSION).zip
	@echo "Creating draft release for $(VERSION). prerelease=$(PRERELEASE)"
	curl $(AUTH) -XPOST $(API_HOST)/repos/$(OWNER)/quickestphp/releases -d '{ \
		"tag_name": "$(VERSION)", \
		"name": "QuickestPHP $(VERSION) released", \
		"draft": true, \
	...
	curl $(AUTH) -XPOST \
		$(UPLOAD_HOST)/repos/$(OWNER)/quickestphp/releases/`cat ./id.txt`/assets?name=quickestphp-$(DASH_VERSION).zip \
		-H "Accept: application/vnd.github.manifold-preview" \
		-H 'Content-Type: application/zip' \
		--data-binary '@dist/quickestphp-$(DASH_VERSION).zip'
	# Cleanup files.
	rm release.json
	rm id.txt

# Tasks for publishing separate repositories out of each QuickestPHP namespace
